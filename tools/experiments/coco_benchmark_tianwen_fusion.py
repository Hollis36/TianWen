#!/usr/bin/env python3
"""
COCO Benchmark using TianWen Framework's Built-in Fusion

Uses the full TianWen fusion system instead of simple verification.
"""

import os
import sys
from pathlib import Path
project_root = Path(__file__).parent.parent.parent
sys.path.insert(0, str(project_root))

import torch
import numpy as np
import json
import time
from PIL import Image
from typing import Dict, List, Tuple
from collections import defaultdict
from tqdm import tqdm

# COCO dataset path
COCO_ROOT = Path(os.environ.get("COCO_ROOT", "./data/coco"))
COCO_IMAGES = COCO_ROOT / "images" / "val2017"
COCO_ANNOTATIONS = COCO_ROOT / "annotations" / "instances_val2017.json"

# Output directory
OUTPUT_DIR = Path("d:/TianWen/coco_benchmark_tianwen")

# COCO class names
COCO_CLASSES = [
    'person', 'bicycle', 'car', 'motorcycle', 'airplane', 'bus', 'train', 'truck', 'boat',
    'traffic light', 'fire hydrant', 'stop sign', 'parking meter', 'bench', 'bird', 'cat',
    'dog', 'horse', 'sheep', 'cow', 'elephant', 'bear', 'zebra', 'giraffe', 'backpack',
    'umbrella', 'handbag', 'tie', 'suitcase', 'frisbee', 'skis', 'snowboard', 'sports ball',
    'kite', 'baseball bat', 'baseball glove', 'skateboard', 'surfboard', 'tennis racket',
    'bottle', 'wine glass', 'cup', 'fork', 'knife', 'spoon', 'bowl', 'banana', 'apple',
    'sandwich', 'orange', 'broccoli', 'carrot', 'hot dog', 'pizza', 'donut', 'cake', 'chair',
    'couch', 'potted plant', 'bed', 'dining table', 'toilet', 'tv', 'laptop', 'mouse',
    'remote', 'keyboard', 'cell phone', 'microwave', 'oven', 'toaster', 'sink', 'refrigerator',
    'book', 'clock', 'vase', 'scissors', 'teddy bear', 'hair drier', 'toothbrush'
]


def load_coco_annotations(ann_file: Path):
    print(f"Loading annotations from {ann_file}...")
    with open(ann_file, 'r') as f:
        coco = json.load(f)

    images = {img['id']: img for img in coco['images']}
    categories = {cat['id']: cat['name'] for cat in coco['categories']}

    annotations = defaultdict(list)
    for ann in coco['annotations']:
        if ann.get('iscrowd', 0) == 0:
            annotations[ann['image_id']].append(ann)

    print(f"  Loaded {len(images)} images, {len(categories)} categories")
    return images, categories, annotations


def bbox_coco_to_xyxy(bbox):
    x, y, w, h = bbox
    return [x, y, x + w, y + h]


def compute_iou(box1, box2):
    x1 = max(box1[0], box2[0])
    y1 = max(box1[1], box2[1])
    x2 = min(box1[2], box2[2])
    y2 = min(box1[3], box2[3])

    inter = max(0, x2 - x1) * max(0, y2 - y1)
    area1 = (box1[2] - box1[0]) * (box1[3] - box1[1])
    area2 = (box2[2] - box2[0]) * (box2[3] - box2[1])
    union = area1 + area2 - inter

    return inter / (union + 1e-6)


def compute_ap(recalls, precisions):
    recalls = np.concatenate([[0], recalls, [1]])
    precisions = np.concatenate([[0], precisions, [0]])

    for i in range(len(precisions) - 2, -1, -1):
        precisions[i] = max(precisions[i], precisions[i + 1])

    recall_thresholds = np.linspace(0, 1, 101)
    ap = 0
    for t in recall_thresholds:
        prec = precisions[recalls >= t]
        ap += prec.max() if len(prec) > 0 else 0
    ap /= 101
    return ap


def evaluate_detections(predictions, ground_truths, num_classes=80):
    all_preds = defaultdict(list)
    all_gts = defaultdict(lambda: defaultdict(list))

    for img_id, pred in enumerate(predictions):
        for box, label, score in zip(pred['boxes'], pred['labels'], pred['scores']):
            box_np = box.cpu().numpy() if torch.is_tensor(box) else box
            all_preds[int(label)].append((img_id, float(score), box_np))

    for img_id, gt in enumerate(ground_truths):
        for box, label in zip(gt['boxes'], gt['labels']):
            box_np = box.cpu().numpy() if torch.is_tensor(box) else box
            all_gts[int(label)][img_id].append(box_np)

    aps_50 = []

    for class_id in range(num_classes):
        preds = sorted(all_preds[class_id], key=lambda x: -x[1])
        gts = all_gts[class_id]

        if len(preds) == 0 or len(gts) == 0:
            continue

        n_gt = sum(len(boxes) for boxes in gts.values())
        gt_matched = {img_id: [False] * len(boxes) for img_id, boxes in gts.items()}
        tp, fp = [], []

        for img_id, score, pred_box in preds:
            if img_id not in gts:
                fp.append(1)
                tp.append(0)
                continue

            gt_boxes = gts[img_id]
            matched = gt_matched[img_id]

            best_iou = 0
            best_idx = -1
            for idx, gt_box in enumerate(gt_boxes):
                if matched[idx]:
                    continue
                iou = compute_iou(pred_box, gt_box)
                if iou > best_iou:
                    best_iou = iou
                    best_idx = idx

            if best_iou >= 0.5 and best_idx >= 0:
                tp.append(1)
                fp.append(0)
                gt_matched[img_id][best_idx] = True
            else:
                tp.append(0)
                fp.append(1)

        tp_cumsum = np.cumsum(tp)
        fp_cumsum = np.cumsum(fp)
        recalls = tp_cumsum / n_gt
        precisions = tp_cumsum / (tp_cumsum + fp_cumsum + 1e-6)

        ap = compute_ap(recalls, precisions)
        aps_50.append(ap)

    return {'mAP@50': np.mean(aps_50) if aps_50 else 0}


def run_tianwen_benchmark(
    num_samples: int = 100,
    vlm_type: str = "qwen2-vl-2b",
    fusion_type: str = "decision_fusion",
):
    """Run benchmark using TianWen framework's fusion."""
    print("=" * 70)
    print(f"TianWen Framework Benchmark: {fusion_type}")
    print(f"VLM: {vlm_type}")
    print("=" * 70)

    OUTPUT_DIR.mkdir(exist_ok=True)

    # Load annotations
    images_meta, categories, annotations = load_coco_annotations(COCO_ANNOTATIONS)
    image_ids = list(images_meta.keys())[:num_samples]

    # COCO ID mapping
    coco_to_yolo = {
        1: 0, 2: 1, 3: 2, 4: 3, 5: 4, 6: 5, 7: 6, 8: 7, 9: 8, 10: 9,
        11: 10, 13: 11, 14: 12, 15: 13, 16: 14, 17: 15, 18: 16, 19: 17, 20: 18, 21: 19,
        22: 20, 23: 21, 24: 22, 25: 23, 27: 24, 28: 25, 31: 26, 32: 27, 33: 28, 34: 29,
        35: 30, 36: 31, 37: 32, 38: 33, 39: 34, 40: 35, 41: 36, 42: 37, 43: 38, 44: 39,
        46: 40, 47: 41, 48: 42, 49: 43, 50: 44, 51: 45, 52: 46, 53: 47, 54: 48, 55: 49,
        56: 50, 57: 51, 58: 52, 59: 53, 60: 54, 61: 55, 62: 56, 63: 57, 64: 58, 65: 59,
        67: 60, 70: 61, 72: 62, 73: 63, 74: 64, 75: 65, 76: 66, 77: 67, 78: 68, 79: 69,
        80: 70, 81: 71, 82: 72, 84: 73, 85: 74, 86: 75, 87: 76, 88: 77, 89: 78, 90: 79,
    }

    # Build TianWen components
    print("\nBuilding TianWen components...")

    from tianwen import DETECTORS, VLMS, FUSIONS

    # Build detector
    print("  Loading detector...")
    detector = DETECTORS.build({
        "type": "yolov8",
        "model_name": "yolov8n",
        "num_classes": 80,
        "pretrained": True,
    })
    detector.eval()
    detector.to("cuda")

    # Build VLM
    print(f"  Loading VLM: {vlm_type}...")
    vlm_model_map = {
        "qwen2-vl-2b": "Qwen/Qwen2-VL-2B-Instruct",
        "qwen2-vl-7b": "Qwen/Qwen2-VL-7B-Instruct",
    }
    vlm = VLMS.build({
        "type": "qwen_vl",
        "model_name": vlm_model_map.get(vlm_type, vlm_type),
        "freeze": True,
    })
    vlm.eval()

    # Build fusion
    print(f"  Setting up fusion: {fusion_type}...")
    fusion = FUSIONS.build(
        {
            "type": fusion_type,
            "verification_mode": "binary",
            "score_adjustment": 0.3,
            "min_confidence": 0.2,
            "max_verifications": 20,
        },
        detector=detector,
        vlm=vlm,
    )
    fusion.class_names = COCO_CLASSES
    fusion.eval()
    fusion.to("cuda")

    print("  All components loaded!")

    # Prepare data transforms
    from torchvision import transforms
    transform = transforms.Compose([
        transforms.ToTensor(),
    ])

    # Run evaluation
    print(f"\nEvaluating on {len(image_ids)} images...")

    detector_predictions = []
    fusion_predictions = []
    ground_truths = []

    detector_time = 0
    fusion_time = 0

    for img_id in tqdm(image_ids, desc="Processing"):
        img_info = images_meta[img_id]
        img_path = COCO_IMAGES / img_info['file_name']

        if not img_path.exists():
            continue

        # Load and transform image
        pil_image = Image.open(img_path).convert('RGB')
        img_tensor = transform(pil_image).unsqueeze(0).to("cuda")

        # Run detector only
        start = time.time()
        with torch.no_grad():
            det_output = detector(img_tensor)
        detector_time += time.time() - start

        # Extract detector predictions
        if det_output.outputs and len(det_output.outputs[0].boxes) > 0:
            det = det_output.outputs[0]
            det_pred = {
                'boxes': det.boxes.cpu(),
                'labels': det.labels.cpu(),
                'scores': det.scores.cpu(),
            }
        else:
            det_pred = {
                'boxes': torch.zeros((0, 4)),
                'labels': torch.zeros(0, dtype=torch.long),
                'scores': torch.zeros(0),
            }
        detector_predictions.append(det_pred)

        # Run fusion
        start = time.time()
        with torch.no_grad():
            fusion_output = fusion(img_tensor)
        fusion_time += time.time() - start

        # Extract fusion predictions
        fused_det = fusion_output.detection_output
        if fused_det.outputs and len(fused_det.outputs[0].boxes) > 0:
            fdet = fused_det.outputs[0]
            fusion_pred = {
                'boxes': fdet.boxes.cpu(),
                'labels': fdet.labels.cpu(),
                'scores': fdet.scores.cpu(),
            }
        else:
            fusion_pred = {
                'boxes': torch.zeros((0, 4)),
                'labels': torch.zeros(0, dtype=torch.long),
                'scores': torch.zeros(0),
            }
        fusion_predictions.append(fusion_pred)

        # Ground truth
        anns = annotations[img_id]
        gt_boxes = []
        gt_labels = []
        for ann in anns:
            bbox = bbox_coco_to_xyxy(ann['bbox'])
            cat_id = ann['category_id']
            if cat_id in coco_to_yolo:
                gt_boxes.append(bbox)
                gt_labels.append(coco_to_yolo[cat_id])

        gt = {
            'boxes': torch.tensor(gt_boxes, dtype=torch.float32) if gt_boxes else torch.zeros((0, 4)),
            'labels': torch.tensor(gt_labels, dtype=torch.long) if gt_labels else torch.zeros(0, dtype=torch.long),
        }
        ground_truths.append(gt)

    # Evaluate
    print("\nEvaluating results...")
    detector_metrics = evaluate_detections(detector_predictions, ground_truths)
    fusion_metrics = evaluate_detections(fusion_predictions, ground_truths)

    # Statistics
    det_total = sum(len(p['boxes']) for p in detector_predictions)
    fusion_total = sum(len(p['boxes']) for p in fusion_predictions)

    det_scores = torch.cat([p['scores'] for p in detector_predictions if len(p['scores']) > 0])
    fusion_scores = torch.cat([p['scores'] for p in fusion_predictions if len(p['scores']) > 0])

    # Print results
    print("\n" + "=" * 70)
    print(f"RESULTS: TianWen {fusion_type} with {vlm_type}")
    print("=" * 70)

    print(f"\n{'Metric':<25} | {'Detector':>12} | {'Fusion':>12} | {'Delta':>12}")
    print("-" * 70)
    print(f"{'mAP@50':<25} | {detector_metrics['mAP@50']:>12.4f} | {fusion_metrics['mAP@50']:>12.4f} | {fusion_metrics['mAP@50']-detector_metrics['mAP@50']:>+12.4f}")
    print("-" * 70)
    print(f"{'Total Detections':<25} | {det_total:>12} | {fusion_total:>12} | {fusion_total-det_total:>+12}")
    print(f"{'Avg Confidence':<25} | {det_scores.mean().item():>12.4f} | {fusion_scores.mean().item():>12.4f} | {fusion_scores.mean().item()-det_scores.mean().item():>+12.4f}")
    print(f"{'Detector FPS':<25} | {len(image_ids)/detector_time:>12.1f} | {'-':>12} | {'-':>12}")
    print(f"{'Fusion FPS':<25} | {'-':>12} | {len(image_ids)/fusion_time:>12.1f} | {'-':>12}")

    # Save results
    results = {
        'vlm_type': vlm_type,
        'fusion_type': fusion_type,
        'num_images': len(image_ids),
        'detector': {
            'mAP@50': float(detector_metrics['mAP@50']),
            'total_detections': det_total,
            'avg_confidence': float(det_scores.mean().item()),
        },
        'fusion': {
            'mAP@50': float(fusion_metrics['mAP@50']),
            'total_detections': fusion_total,
            'avg_confidence': float(fusion_scores.mean().item()),
        },
    }

    with open(OUTPUT_DIR / f"results_{fusion_type}_{vlm_type.replace('/', '_')}.json", 'w') as f:
        json.dump(results, f, indent=2)

    print(f"\nResults saved to {OUTPUT_DIR}")
    print("=" * 70)

    return results


if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser()
    parser.add_argument("--samples", type=int, default=50)
    parser.add_argument("--vlm", type=str, default="qwen2-vl-2b")
    parser.add_argument("--fusion", type=str, default="decision_fusion",
                       choices=["decision_fusion", "prompt_guided"])
    args = parser.parse_args()

    results = run_tianwen_benchmark(
        num_samples=args.samples,
        vlm_type=args.vlm,
        fusion_type=args.fusion,
    )
