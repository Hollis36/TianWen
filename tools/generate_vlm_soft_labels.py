#!/usr/bin/env python3
"""
VLM Soft Label Generation Script

Generate soft labels (VLM verification scores) for detector predictions.
These labels will be used for knowledge distillation training.
"""
...
# Output directory
OUTPUT_DIR = Path("d:/TianWen/soft_labels")
...
class VLMVerifier:
    """VLM-based detection verifier for generating soft labels."""
    ...
    def verify_detection(
        ...
        """
        Use VLM to verify a detection and return soft label.

        Returns:
            soft_label: 0.0 (reject) to 1.0 (confident accept)
        """
        ...
        try:
            ...
            # Parse response to soft label
            if "yes" in response:
                # VLM confirms - combine with detector confidence
                soft_label = min(1.0, confidence * 0.7 + 0.3)  # Weighted combination
            elif "no" in response:
                # VLM rejects
                soft_label = max(0.0, confidence * 0.3)  # Heavily penalize
            else:
            ...
            print(f"   VLM error: {e}")
            return confidence  # Return original confidence on error
    ...
    def verify_batch(
        ...
        """
        Verify all detections in an image and return soft labels.

        Only verify detections below verify_threshold to save time.
        High confidence detections keep their original score.
        """
        num_dets = len(boxes)
        if num_dets == 0:
...
def generate_soft_labels(
    ...
    """Generate VLM soft labels for detector predictions."""

    print("=" * 70)
    print(f"Generating VLM Soft Labels")
    print(f"  Split: {split}")
    ...
    soft_labels_data = {}
    checkpoint_file = OUTPUT_DIR / f"soft_labels_{split}_{vlm_type.replace('/', '_')}_checkpoint.pkl"
    ...
    # Save final results
    output_file = OUTPUT_DIR / f"soft_labels_{split}_{vlm_type.replace('/', '_')}.pkl"
    ...
    summary_file = OUTPUT_DIR / f"soft_labels_{split}_{vlm_type.replace('/', '_')}_summary.json"
